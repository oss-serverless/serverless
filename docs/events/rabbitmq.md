...
Note: OSLS Framework only sets this property if you explicitly add it to the `rabbitmq` configuration (see an example below). The following example will only process records that are published in to Amazon MQ for RabbitMQ where field `a` is equal to 1 or 2.
