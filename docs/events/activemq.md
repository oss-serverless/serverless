...
Note: osls only sets this property if you explicitly add it to the `activemq` configuration (see an example below). The following example will only process records that are published to Amazon MQ for Apache ActiveMQ where field `a` is equal to 1 or 2.
