...
- [Simple event definition](#simple-event-definition)
  - [Including the request body](#including-the-request-body)
  - [Cache Behavior configuration](#cache-behavior-configuration)
  - [Cache Policy configuration](#cache-policy-configuration)
  - [CloudFront Distribution configurations](#cloudfront-distribution-configurations)
...
If there is no behavior with an empty `pathPattern` in the serverless configuration, osls will create an additional behavior with an empty `pathPattern` pointing to the defined origin.
...
Set `includeBody: true` to forward the request body to the Lambda@Edge function. This is only supported for the `viewer-request` and `origin-request` event types, and lets the function read (and, for origin requests, modify) the body of the incoming request. See the [AWS documentation on accessing the request body](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/lambda-include-body-access.html).
...
The default values for behaviors as generated by osls are
...
**Note:** By default osls assigns the AWS Managed [`Managed-CachingOptimized`](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/using-managed-cache-policies.html#managed-cache-policies-list) cache policy to each behavior. Setting any of `behavior.ForwardedValues`, `behavior.MaxTTL`, `behavior.MinTTL`, or `behavior.DefaultTTL` (which configure legacy cache settings) suppresses that default cache-policy assignment, because a behavior cannot use a cache policy and these legacy settings at the same time. Referencing a cache policy explicitly via `behavior.CachePolicyId` or the `cachePolicy` property (see below) also overrides the default.
