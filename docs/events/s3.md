...
Amazon S3 can invoke a Lambda function whenever objects are created, removed, or otherwise changed in a bucket. osls configures the bucket notification and the required permissions so your function runs in response to those events. See the [AWS S3 event notifications documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/NotificationHowTo.html).

## Simple event definition

This will create a `photos` bucket which fires the `resize` function when an object is added or modified inside the bucket. A hardcoded bucket name can lead to issues as a bucket name can only be used once in S3. For that you can use the [osls variable syntax](../guides/variables.md) and add dynamic elements to the bucket name.
