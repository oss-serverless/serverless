...
To create HTTP endpoints as Event sources for your AWS Lambda Functions, use OSLS Framework's easy AWS API Gateway Events syntax.
...
By default, OSLS Framework deploys your REST API using the EDGE endpoint configuration. If you would like to use the REGIONAL or PRIVATE configuration, set the `endpointType` parameter in your `provider` block.
...
OSLS Framework ships with the following default request templates you can use out of the box:
...
OSLS Framework lets you setup custom headers and a response template for your `http` event.
...
OSLS Framework ships with default status codes you can use to e.g. signal that a resource could not be found (404) or that
...
You can override the defaults status codes supplied by OSLS Framework. You can use this to change the default status code, add/remove status codes, or change the templates and headers used for each status code. Use the pattern key to change the selection process that dictates what code is returned.
...
provider:
  name: aws
  apiGateway:
    restApiId: xxxxxxxxxx # REST API resource ID. Default is generated by OSLS Framework
    restApiRootResourceId: xxxxxxxxxx # Root resource, represent as / path
    websocketApiId: xxxxxxxxxx # Websocket API resource ID. Default is generated by OSLS Framework
...
You can define more than one path resource, but by default, OSLS Framework will generate them from the root resource.
...
- Let OSLS Framework create and assign an IAM role for you (default behavior). Note that since this is a shared setting, this role is not removed when you remove the deployment.
- Let OSLS Framework assign an existing IAM role that you created before the deployment, if not already assigned:
...
- Do not let OSLS Framework manage the CloudWatch role configuration. In this case, you would create and assign the IAM role yourself, e.g. in a separate "account setup" deployment:

  ```yml
  provider:
    logs:
      restApi:
        roleManagedExternally: true # disables automatic role creation/checks done by OSLS Framework
  ```

**Note:** OSLS Framework configures the API Gateway CloudWatch role setting using a custom resource lambda function. If you're using `iam.deploymentRole` to specify a limited-access IAM role for your serverless deployment, the custom resource lambda will assume this role during execution.
