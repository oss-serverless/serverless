...
- `--durable-execution-name` Unique name for a durable function execution. Durable functions require an explicit `--qualifier`; use `--qualifier durable` for osls-managed durable functions. Execution names must be 1-64 characters and can contain alphanumeric characters, hyphens, or underscores.
...
> **Security note:** For `osls invoke local`, `.js` files provided with `--path` or `--contextPath` are loaded with Node.js and executed. Treat those files and any paths supplied by scripts or CI as trusted input. Prefer JSON or YAML files for event and context data from untrusted sources.
...
```bash
osls invoke --function functionName --stage dev --region us-east-1
```
...
```bash
osls invoke --function functionName --data "hello world"
```

#### Function invocation with custom context

```bash
osls invoke --function functionName --context "hello world"
```

#### Function invocation with context passing

```bash
osls invoke --function functionName --contextPath lib/context.json
```
...
```bash
node dataGenerator.js | osls invoke --function functionName
```

#### Function invocation with logging

```bash
osls invoke --function functionName --log
```
...
```bash
osls invoke --function functionName --path lib/data.json
```
