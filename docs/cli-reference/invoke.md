...
To invoke a function locally, see [`osls invoke local`](./invoke-local.md).
...
Lambda functions assume an _IAM role_ during execution: osls creates this role, and set all the permission provided in the `iam.role.statements` section of `serverless.yml`.
