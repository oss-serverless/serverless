    ...
    resolve: async ({ address }) => {
      ...
      const result = await (async () => {
        try {
          ...
          if (provider._v3Enabled) {
            return await provider.requestV3('S3', 'getObject', { Bucket: bucketName, Key: key }, { useCache: true });
          }
          return await provider.request('S3', 'getObject', { Bucket: bucketName, Key: key }, { useCache: true });
        } catch (error) {
