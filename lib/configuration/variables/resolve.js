...
const isWriteTarget = (value) =>
  value != null && (typeof value === 'object' || typeof value === 'function');

const getOwnPathTarget = (source, pathKeys) => {
  let current = source;
...
class VariablesResolver {
  constructor({
    ...
    // Matching properties are not scheduled for resolution in this pass.
    // They remain in `variablesMeta`, so they still resolve on demand when another property
    // depends on them (see `resolveDependentProperty`)
  ...
  async resolveInternalResult(resolutionBatchId, propertyPath, valueMeta, nestTracker = 10) {
    ...
    for (const [propertyKeyPath, propertyValueMeta] of propertyVariablesMeta) {
      ...
      const targetObject = getOwnPathTarget(valueMeta.value, propertyKeyPathKeys.slice(0, -1));

      if (!isWriteTarget(targetObject)) continue;

      safeSet(targetObject, targetKey, propertyValueMeta.value);
    }
  }
...
Object.defineProperties(
  ...
  memoizeMethods({
    resolveProperty: methodDescriptor(
      async function self(resolutionBatchId, propertyPath) {
        ...
        const targetObject = getOwnPathTarget(this.configuration, propertyPathKeys.slice(0, -1));

        if (!isWriteTarget(targetObject)) return;

        safeSet(targetObject, targetKey, value);
        ...
        // Deferred properties (e.g. params of irrelevant stages discovered in a section resolved
        // from a single variable) are registered above but not scheduled: they still resolve on
        // demand when another property depends on them (see `resolveDependentProperty`)
    ...
    resolveSource: methodDescriptor(
