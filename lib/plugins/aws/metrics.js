...
class AwsMetrics {
  ...
  /**
   * Helper method to route CloudWatch requests through v2 or v3 based on feature flag
   * @private
   */
  async _cloudWatchRequest(method, params) {
    // Use v3 if feature flag is enabled, otherwise fallback to v2
    if (this.provider._v3Enabled) {
      return this.provider.requestV3('CloudWatch', method, params);
    }
    return this.provider.request('CloudWatch', method, params);
  }
  ...
  async getMetrics() {
    ...
    return Promise.all(
      functions.map(async (functionName) => {
        ...
        const getMetrics = (params) => this._cloudWatchRequest('getMetricStatistics', params);
