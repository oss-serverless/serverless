...
module.exports = {
  async uploadZipFile({ filename, s3KeyDirname, basename }) {
    ...
    // Declared explicitly as the SDK otherwise sizes the stream with lstat, which for a
    // symlinked artifact reports the link itself rather than the file being streamed
    const { size: contentLength } = await fsp.stat(filename);
