...
module.exports = {
  async createFallback() {
    ...
    if (this.serverless.service.provider.deploymentMethod === 'direct') {
      ...
      monitorCfData = await this.provider._awsRequest('CloudFormation', 'createStack', params);
    } else {
      ...
      await this.provider._awsRequest('CloudFormation', 'createChangeSet', createChangeSetParams);
      ...
      if (isChangeSetWithoutChanges(changeSetDescription)) {
        ...
        await this.provider._awsRequest('CloudFormation', 'deleteChangeSet', {
          StackName: stackName,
      ...
      await this.provider._awsRequest('CloudFormation', 'executeChangeSet', executeChangeSetParams);
  ...
  async update() {
    ...
    if (this.serverless.service.provider.deploymentMethod === 'direct') {
      ...
      try {
        monitorCfData = await this.provider._awsRequest('CloudFormation', 'updateStack', params);
      } catch (e) {
      ...
      await this._cloudFormationRequest('deleteChangeSet', {
        StackName: stackName,
        ChangeSetName: changeSetName,
      });
    ...
    if (
      ...
      await this.provider._awsRequest('CloudFormation', 'setStackPolicy', {
        StackName: stackName,
