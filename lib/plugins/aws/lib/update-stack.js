...
module.exports = {
  ...
  async createFallback() {
    ...
    if (this.serverless.service.provider.deploymentMethod === 'direct') {
      ...
      monitorCfData = await this._cloudFormationRequest('createStack', params);
    } else {
      ...
      await this._cloudFormationRequest('createChangeSet', createChangeSetParams);
      ...
      await this._cloudFormationRequest('executeChangeSet', executeChangeSetParams);
  ...
  async update() {
    ...
    if (this.serverless.service.provider.deploymentMethod === 'direct') {
      ...
      try {
        monitorCfData = await this._cloudFormationRequest('updateStack', params);
      } catch (e) {
    ...
    if (
      ...
      await this._cloudFormationRequest('setStackPolicy', {
        StackName: stackName,
