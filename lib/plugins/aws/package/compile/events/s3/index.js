  ...
  constructor(serverless, options) {
    ...
    this.hooks = {
      'package:compileEvents': async () => {
        await this.newS3Buckets();
        return this.existingS3Buckets();
      },
    };
  }
      ...
      if (functionObj.events) {
        functionObj.events.forEach((event) => {
          if (event.s3) {
            ...
            let bucketName;
            if (hasOwn(providerS3, bucketRef)) {
      ...
      ([bucketName, bucketLambdaConfiguration]) => {
        ...
        if (bucketsMeta[bucketName]) {
          ...
          for (const [key, value] of Object.entries(providerBucket)) {
            if (key !== 'name') {
              const property = this.provider.naming.normalizeName(key);
              safeSet(bucketConf, property, value);
            }
          }
        }
        let providedNotificationConfiguration = {};
        if (bucketConf && hasOwn(bucketConf, 'NotificationConfiguration')) {
        ...
        const bucketTemplate = {
          Type: 'AWS::S3::Bucket',
          Properties: safeShallowAssign(
            {
              BucketName: bucketName,
              NotificationConfiguration: safeShallowAssign(
                {
        ...
        dependsOnToCreate.forEach((item) => {
          ...
          const dependsOn = functionObj.targetAlias && functionObj.targetAlias.logicalId;
          if (dependsOn) {
        ...
        functionObj.events.forEach((event) => {
          if (event.s3 && isObject(event.s3) && event.s3.existing) {
            ...
            rules = (event.s3.rules || []).map((rule) => {
              ...
              return {
                [this.provider.naming.toStartCase(key)]: value,
              };
            });
            ...
            if (bucketResources[stringifiedBucket]) {
              bucketResources[stringifiedBucket] = Array.from(
                new Set([...bucketResources[stringifiedBucket], customS3ResourceLogicalId])
              );
            } else {
            ...
            if (customS3Resource) {
              Object.assign(Resources, customS3Resource);
            }
          }
        });
      }
