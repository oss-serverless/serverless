...
const { hasOwn, safeSet, safeShallowAssign } = require('../../../../../utils/safe-object');
...
class AwsCompileCloudFrontEvents {
  ...
  compileCloudFrontEvents() {
    ...
    const uniqWith = (items, comparator) => {
      const uniqueItems = [];
      for (const item of items) {
        if (uniqueItems.some((candidate) => comparator(item, candidate))) continue;
    ...
    function extendDeep(object, source) {
      for (const [key, sourceValue] of Object.entries(source)) {
        const hasOwnBranch = hasOwn(object, key);
        const targetValue = hasOwnBranch ? object[key] : undefined;

        if (Array.isArray(targetValue) && Array.isArray(sourceValue)) {
          safeSet(object, key, uniqWith(targetValue.concat(sourceValue), isDeepStrictEqual));
          continue;
        }

        if (hasOwnBranch && isObject(targetValue) && isObject(sourceValue)) {
          extendDeep(targetValue, sourceValue);
          continue;
        }

        if (!hasOwnBranch || targetValue === undefined) {
          safeSet(object, key, structuredClone(sourceValue));
        }
      }
    ...
    function createOrigin(origin, naming) {
      ...
      if (typeof origin === 'string') {
        ...
        safeShallowAssign(originObj, origin);
      }
          ...
          if (event.cloudFront) {
            ...
            const lambdaVersionLogicalId = Object.keys(Resources).find((key) => {
              ...
              return (
                ...
                resource.Properties.FunctionName.Ref === lambdaFunctionLogicalId
              );
            });
            ...
            if (event.cloudFront.behavior) {
              safeShallowAssign(behavior, event.cloudFront.behavior);
            }
            ...
            lambdaAtEdgeFunctions.push(
              safeShallowAssign({}, functionObj, { functionName, lambdaVersionLogicalId })
            );
          }
        });
      }
    ...
    if (lambdaAtEdgeFunctions.length) {
      ...
      if (behaviors.length !== uniqBy(behaviors, 'PathPattern').length) {
        throw new ServerlessError(
      ...
      if (
        behaviors.some((o) => {
          return (
            ...
            uniqBy(o.LambdaFunctionAssociations, 'EventType').length
          );
        })
      ) {
        throw new ServerlessError(
      ...
      if (behaviors[0].PathPattern) {
        ...
        const behavior = Object.fromEntries(
          Object.entries(behaviors[0]).filter(
            ([key]) => key !== 'PathPattern' && key !== 'LambdaFunctionAssociations'
          )
        );
