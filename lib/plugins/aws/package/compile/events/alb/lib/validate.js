...
const collectDuplicates = (items, comparator) => {
  const uniqueItems = [];
  const duplicates = [];

  items.forEach((item) => {
    if (uniqueItems.some((candidate) => comparator(item, candidate))) {
...
module.exports = {
  ...
  validate() {
    const authorizers = createRegistry();
  ...
  validatePriorities(albEvents) {
    ...
    duplicates = collectDuplicates(albEvents, comparator);
