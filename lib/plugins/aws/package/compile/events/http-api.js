...
class HttpApiEvents {
  ...
  compileApi() {
    ...
    if (this.serverless.service.provider.tags) {
      const tags = safeShallowAssign({}, this.serverless.service.provider.tags);
  ...
  compileStage() {
    ...
    if (this.serverless.service.provider.tags) {
      properties.Tags = safeShallowAssign({}, this.serverless.service.provider.tags);
    }
  ...
  compileAuthorizers() {
    for (const authorizer of this.config.authorizers.values()) {
      ...
      if (authorizer.type === 'request') {
        ...
        authorizerResource.DependsOn =
          authorizer.functionObject &&
          authorizer.functionObject.targetAlias &&
          authorizer.functionObject.targetAlias.logicalId;
...
Object.defineProperties(
  ...
  memoizeeMethods({
    resolveConfiguration: methodDescriptor(function () {
        ...
        for (const event of functionData.events) {
          ...
          if (authorizer) {
            const { name, scopes, id, type } = (() => {
              if (isObject(authorizer)) return authorizer;
    ...
    compileIntegration: methodDescriptor(function (routeTargetData) {
      ...
      this.cfTemplate.Resources[
        ...
        DependsOn: routeTargetData.functionAlias && routeTargetData.functionAlias.logicalId,
        Properties: properties,
      };
    }),
    compileLambdaPermissions: methodDescriptor(function (routeTargetData) {
      this.cfTemplate.Resources[
