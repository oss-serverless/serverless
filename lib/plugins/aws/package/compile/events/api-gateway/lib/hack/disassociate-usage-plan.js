  ...
  async disassociateUsagePlan() {
    ...
    if (apiKeys && apiKeys.length) {
      ...
      return BbPromise.all([
        this.provider._awsRequest('CloudFormation', 'describeStackResource', {
          StackName: stackName,
          LogicalResourceId: this.provider.naming.getRestApiLogicalId(),
        }),
        this.provider._awsRequest('APIGateway', 'getUsagePlans', {}),
      ])
        .then((data) =>
            ...
            items
              .map((item) =>
                item.apiStages.map((apiStage) =>
                  this.provider._awsRequest('APIGateway', 'updateUsagePlan', {
                    usagePlanId: item.id,
                    patchOperations: [
                      {
                        op: 'remove',
