...
module.exports = {
  ...
  getIntegrationResponses(http) {
    ...
    const mapHeaders = (headers) =>
      Object.fromEntries(
        Object.entries(headers).map(([header, value]) => [`method.response.header.${header}`, value])
      );
    ...
    if (http.response) {
      ...
      if (http.response.headers) {
        Object.assign(integrationResponseHeaders, http.response.headers);
      }
      ...
      Object.entries(http.response.statusCodes).forEach(([statusCode, config]) => {
        ...
        if (config.template) {
          ...
          Object.assign(integrationResponse.ResponseTemplates, template);
        }
