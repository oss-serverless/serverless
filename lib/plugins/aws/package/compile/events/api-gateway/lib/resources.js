...
const { createRegistry, hasOwn, safeSet } = require('../../../../../../../utils/safe-object');
...
module.exports = {
  ...
  getResourcePaths() {
    ...
    if (
      ...
      return predefinedResources.reduce((resourceMap, resource) => {
        const r = resourceMap;
        safeSet(r, resource.path, resource);
  ...
  getResourceId(path) {
    ...
    if (!this.apiGatewayResources || !hasOwn(this.apiGatewayResources, path)) {
  ...
  getResourceName(path) {
    ...
    if (!hasOwn(this.apiGatewayResources, path)) return undefined;
