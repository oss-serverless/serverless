...
async function resolveRestApiId() {
  return new BbPromise((resolve) => {
    ...
    const resolveFromAws = (position) =>
      this.provider
        ._awsRequest('APIGateway', 'getRestApis', { position, limit: 500 })
        .then((result) => {
...
async function resolveDeploymentId() {
  if (!Object.keys(this.apiGatewayStageState).length) {
    ...
    return this.provider
      ._awsRequest('APIGateway', 'getDeployments', {
        restApiId,
        limit: 500,
      })
      .then((res) => {
...
async function ensureStage() {
  if (!Object.keys(this.apiGatewayStageState).length) {
    ...
    return this.provider._awsRequest('APIGateway', 'createStage', {
      deploymentId,
      restApiId,
      stageName: this.provider.getApiGatewayStage(),
    });
...
async function addTags() {
  const requests = this.apiGatewayTagResourceParams.map((tagResourceParam) =>
    this.provider._awsRequest('APIGateway', 'tagResource', tagResourceParam)
  );
...
async function removeTags() {
  const requests = this.apiGatewayUntagResourceParams.map((untagResourceParam) =>
    this.provider._awsRequest('APIGateway', 'untagResource', untagResourceParam)
  );
  return BbPromise.all(requests);
}
...
function applyUpdates() {
  ...
  if (patchOperations.length) {
    return this.provider._awsRequest('APIGateway', 'updateStage', {
      restApiId,
      stageName: this.provider.getApiGatewayStage(),
      patchOperations,
    });
...
async function removeAccessLoggingLogGroup() {
  ...
  if (!accessLogging) {
    return this.provider
      ._awsRequest('CloudWatchLogs', 'deleteLogGroup', {
        logGroupName,
      })
      .catch(() => {
        // fail silently...
