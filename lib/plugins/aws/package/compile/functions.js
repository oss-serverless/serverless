...
// INVARIANT: this set must list every function event key that compiles to an
// AWS::Lambda::EventSourceMapping (poll-based; invokes durable functions synchronously).
// If a new ESM-backed event type is added (e.g. DocumentDB change streams), add its key
// here so the 900s synchronous-ESM durable executionTimeout cap keeps applying.
const durableEventSourceMappingEventKeys = new Set([
  'sqs',
  'stream',
  'msk',
  'kafka',
...
const durableEventSourceMappingMaxExecutionTimeout = 900;

// https://docs.aws.amazon.com/lambda/latest/dg/durable-supported-runtimes.html
const durableSupportedManagedRuntimes = new Set([
  'nodejs22.x',
  'nodejs24.x',
  'python3.13',
  'python3.14',
...
const durableSnapStartSupportedManagedRuntimes = new Set([
  'python3.13',
  'python3.14',
  'java17',
  'java21',
...
class AwsCompileFunctions {
  ...
  async compileFunction(functionName) {
    ...
    if (functionObject.durableConfig) {
      this.validateDurableFunctionConfig(functionName, functionObject);
    }
    ...
    if (functionObject.durableConfig) {
      functionResource.Properties.DurableConfig = {
      ...
      if (functionObject.durableConfig.retentionPeriodInDays !== undefined) {
        functionResource.Properties.DurableConfig.RetentionPeriodInDays =
          functionObject.durableConfig.retentionPeriodInDays;
      }
    }

    const shouldVersionFunction =
      Boolean(functionObject.durableConfig) ||
    ...
    if (
      ...
      } else if (functionObject.provisionedConcurrency) {
  ...
  validateDurableFunctionConfig(functionName, functionObject) {
    let runtime;

    if (!functionObject.image) {
      runtime = this.provider.getRuntime(functionObject.runtime);
      if (!durableSupportedManagedRuntimes.has(runtime)) {
        throw new ServerlessError(
          `Function "${functionName}" cannot configure "durableConfig" with unsupported runtime "${runtime}". AWS Lambda Durable Functions support nodejs22.x, nodejs24.x, python3.13, python3.14, java17, java21, and java25 managed runtimes, or compatible container images.`,
          'DURABLE_FUNCTION_UNSUPPORTED_RUNTIME'
        );
      }
    }

    if (functionObject.snapStart) {
      this.validateDurableSnapStartConfig(functionName, functionObject, runtime);
    }

    if (functionObject.destinations) {
      throw new ServerlessError(
        `Function "${functionName}" cannot configure "destinations" with "durableConfig". AWS Lambda Durable Functions do not support Lambda destinations.`,
        'DURABLE_FUNCTION_DESTINATIONS_UNSUPPORTED'
      );
    }

    const functionEvents = (functionObject.events || []).filter(isObject);

    const hasEventSourceMapping = functionEvents.some((event) =>
      Object.keys(event).some((eventKey) => durableEventSourceMappingEventKeys.has(eventKey))
    );

    if (
      hasEventSourceMapping &&
      functionObject.durableConfig.executionTimeout > durableEventSourceMappingMaxExecutionTimeout
    ) {
      throw new ServerlessError(
        `Function "${functionName}" uses an event source mapping with "durableConfig.executionTimeout" greater than 900 seconds. AWS Lambda event source mappings invoke durable functions synchronously and require durable execution timeout to be 15 minutes or less.`,
        'DURABLE_FUNCTION_ESM_TIMEOUT_UNSUPPORTED'
      );
    }

    if (functionEvents.some((event) => event.cloudFront)) {
      throw new ServerlessError(
        `Function "${functionName}" cannot configure "durableConfig" with a "cloudFront" event. AWS Lambda@Edge does not support durable functions.`,
        'DURABLE_FUNCTION_LAMBDA_EDGE_UNSUPPORTED'
      );
    }

    if (functionObject.maximumRetryAttempts != null) {
      throw new ServerlessError(
        `Function "${functionName}" cannot configure "maximumRetryAttempts" with "durableConfig". AWS Lambda asynchronous retry attempts do not apply to durable executions.`,
        'DURABLE_FUNCTION_MAXIMUM_RETRY_ATTEMPTS_UNSUPPORTED'
      );
    }
  }

  validateDurableSnapStartConfig(functionName, functionObject, runtime) {
    if (functionObject.image) {
      throw new ServerlessError(
        `Function "${functionName}" cannot configure both "durableConfig" and "snapStart" when packaged as a container image. AWS Lambda SnapStart does not support container images.`,
        'DURABLE_FUNCTION_SNAPSTART_UNSUPPORTED'
      );
    }

    if (!durableSnapStartSupportedManagedRuntimes.has(runtime)) {
      throw new ServerlessError(
        `Function "${functionName}" cannot configure both "durableConfig" and "snapStart" with runtime "${runtime}". Supported durable SnapStart runtimes are python3.13, python3.14, java17, java21, and java25.`,
        'DURABLE_FUNCTION_SNAPSTART_UNSUPPORTED'
      );
    }

    if (functionObject.fileSystemConfig) {
      throw new ServerlessError(
        `Function "${functionName}" cannot configure both "durableConfig" and "snapStart" with "fileSystemConfig". AWS Lambda SnapStart does not support EFS.`,
        'DURABLE_FUNCTION_SNAPSTART_UNSUPPORTED'
      );
    }

    if (functionObject.ephemeralStorageSize > 512) {
      throw new ServerlessError(
        `Function "${functionName}" cannot configure both "durableConfig" and "snapStart" with ephemeralStorageSize greater than 512 MB.`,
        'DURABLE_FUNCTION_SNAPSTART_UNSUPPORTED'
      );
    }
  }
  ...
  initializeTargetAlias(functionName, functionObject) {
    if (functionObject.durableConfig) {
      functionObject.targetAlias = {
        name: this.provider.naming.getLambdaDurableAliasName(),
        logicalId: this.provider.naming.getLambdaDurableAliasLogicalId(functionName),
      };
    } else if (functionObject.provisionedConcurrency) {
      functionObject.targetAlias = {
