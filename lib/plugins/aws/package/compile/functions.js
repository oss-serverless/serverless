...
class AwsCompileFunctions {
  ...
  async compileFunction(functionName) {
    ...
    if (tracing) {
      ...
      // update the PolicyDocument statements (if default policy is used)
      if (iamRoleLambdaExecution) {
        appendIfMissing(iamRoleLambdaExecution.Properties.Policies[0].PolicyDocument.Statement, stmt);
      }
    }
    ...
    if (
      ...
      if (lambdaHashingVersion < 20201221 && !this.options['enforce-hash-update']) {
        ...
        for (const { name, properties: layerProperties } of layerConfigurations) {
          sortedLayerConfigurations[name] = Object.fromEntries(Object.entries(layerProperties).sort(byKey));
        }
        functionProperties.layerConfigurations = sortedLayerConfigurations;
        const sortedFunctionProperties = Object.fromEntries(Object.entries(functionProperties).sort(byKey));
