...
module.exports = {
  mergeIamTemplates() {
    ...
    // NOTE: this Allow is merged into the shared role's single inline PolicyDocument,
    // which cannot carry a per-statement CloudFormation Condition. A durable function
    // excluded by a `condition` therefore still contributes a harmless, tightly-scoped
    // resource entry for an ARN that will not exist unless the condition is true.
    const durableExecutionResources = this.serverless.service
      .getAllFunctions()
      .map((functionName) => this.serverless.service.getFunction(functionName))
      .filter((functionObject) => {
        return (
          functionObject.durableConfig &&
          this.provider.resolveFunctionIamRoleResourceName(functionObject) ===
            this.provider.naming.getRoleLogicalId()
        );
      })
      .map((functionObject) => ({
        'Fn::Sub': `arn:\${AWS::Partition}:lambda:\${AWS::Region}:\${AWS::AccountId}:function:${functionObject.name}:*/durable-execution/*/*`,
      }));

    if (durableExecutionResources.length) {
      this.mergeStatements([
        {
          Effect: 'Allow',
          Action: ['lambda:CheckpointDurableExecution', 'lambda:GetDurableExecutionState'],
          Resource: durableExecutionResources,
        },
      ]);
    }
