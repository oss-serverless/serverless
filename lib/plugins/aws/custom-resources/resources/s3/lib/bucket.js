...
async function updateConfiguration(config) {
  const { lambdaArn, functionName, previousFunctionName, bucketName, bucketConfigs, region } =
    config;
    ...
    if (
      ...
      NotificationConfiguration.LambdaFunctionConfigurations =
        NotificationConfiguration.LambdaFunctionConfigurations.filter(
          (conf) =>
            !isOwnedConfiguration(conf, functionName) &&
            !(previousFunctionName && isOwnedConfiguration(conf, previousFunctionName))
        );
    }
