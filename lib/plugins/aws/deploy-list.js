...
class AwsDeployList {
  ...
  async listDeployments() {
    ...
    let response;
    try {
      response = await this._awsRequest('S3', 'listObjectsV2', {
  ...
  async getFunctions() {
    ...
    const result = await Promise.all(
      funcs.map((funcName) => {
        ...
        return this._awsRequest('Lambda', 'getFunction', params);
      })
    );
  ...
  async getFunctionPaginatedVersions(params, totalVersions) {
    const response = await this._awsRequest('Lambda', 'listVersionsByFunction', params);
