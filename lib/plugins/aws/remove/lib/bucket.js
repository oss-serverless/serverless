...
module.exports = {
  ...
  async listObjectsV2() {
    ...
    let result;
    try {
      result = await this.provider._awsRequest('S3', 'listObjectsV2', {
  ...
  async listObjectVersions() {
    ...
    const result = await this.provider._awsRequest('S3', 'listObjectVersions', {
      Bucket: this.bucketName,
      Prefix: `${this.provider.getDeploymentPrefix()}/${serviceStage}`,
    });
  ...
  async deleteObjects() {
    if (this.objectsInBucket.length) {
      const data = await this.provider._awsRequest('S3', 'deleteObjects', {
        Bucket: this.bucketName,
        Delete: {
