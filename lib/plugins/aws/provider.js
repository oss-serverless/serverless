      ...
      serverless.configSchemaHandler.defineProvider('aws', {
        ...
        function: {
          properties: {
            ...
            vpc: {
              // `false`/`null` opt a function out of `provider.vpc`; the compile
              // step (package/compile/functions.js) handles both, so the schema
              // must accept them too (otherwise strict validation rejects them).
              anyOf: [
