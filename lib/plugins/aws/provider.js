...
class AwsProvider {
  ...
  /**
   * Primary legacy AWS request interface backed by AWS SDK v2.
  ...
  /**
   * Direct AWS SDK v2 interface (private)
   * @param {string} service - Service name
   * @param {string} method - Method name
   * @param {Object} params - Parameters
  ...
  async _requestV2(service, method, params, options) {
    ...
    const shouldCache = requestOptions.useCache === undefined ? false : requestOptions.useCache;
    ...
    const serviceOptions = {
      ...
      params: {
        ...credentials,
        region: requestOptions.region === undefined ? this.getRegion() : requestOptions.region,
  ...
  async getAwsSdkV3Config(options = {}) {
    const { credentials, profile, region, service, ...clientOptions } = options;

    if (
      service === 'S3' &&
      clientOptions.useAccelerateEndpoint === undefined &&
      this.isS3TransferAccelerationEnabled()
    ) {
      clientOptions.useAccelerateEndpoint = true;
    }
