...
class AwsProvider {
  constructor(serverless, options) {
    ...
    this._v3Enabled = process.env.SLS_AWS_SDK_V3 === 'true'; // Feature flag for gradual rollout
  ...
  /**
   * AWS SDK v3 request method - new interface for gradual migration
   ...
   */
  async requestV3(service, method, params = {}, options = {}) {
    try {
   ...
   * @returns {Promise} AWS API response
   */
  async _awsRequest(service, method, params, options) {
    // Use v3 if feature flag is enabled, otherwise fallback to v2
    if (this._v3Enabled) {
      return this.requestV3(service, method, params, options);
    }
    return this.request(service, method, params, options);
  }
  ...
  async getServerlessDeploymentBucketName() {
    ...
    const requestMethod = this._v3Enabled ? 'requestV3' : 'request';
    return this[requestMethod]('CloudFormation', 'describeStackResource', {
  ...
  getStackResources(next, resourcesParam) {
    ...
    const requestMethod = this._v3Enabled ? 'requestV3' : 'request';
    return this[requestMethod]('CloudFormation', 'listStackResources', params).then((res) => {
...
Object.defineProperties(
  ...
  memoizeeMethods({
    getAccountInfo: d(
      async function () {
        const requestMethod = this._v3Enabled ? 'requestV3' : 'request';
        const result = await this[requestMethod]('STS', 'getCallerIdentity', {});
    ...
    dockerLoginToEcr: d(
      async function () {
        ...
        const requestMethod = this._v3Enabled ? 'requestV3' : 'request';
        const result = await this[requestMethod]('ECR', 'getAuthorizationToken', {
    ...
    getOrCreateEcrRepository: d(
      async function (scanOnPush) {
        ...
        try {
          const requestMethod = this._v3Enabled ? 'requestV3' : 'request';
          const result = await this[requestMethod]('ECR', 'describeRepositories', {
          ...
          const requestMethod = this._v3Enabled ? 'requestV3' : 'request';
          const result = await this[requestMethod]('ECR', 'createRepository', {
            repositoryName,
    ...
    resolveImageUriAndShaFromUri: d(
      async function (image) {
        ...
        const requestMethod = this._v3Enabled ? 'requestV3' : 'request';
        const describeImagesResponse = await this[requestMethod]('ECR', 'describeImages', {
          imageIds: [
            {
              imageTag,
            },
