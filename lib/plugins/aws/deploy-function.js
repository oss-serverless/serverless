...
class AwsDeployFunction {
  constructor(serverless, options) {
    ...
    this.hooks = {
      ...
      'deploy:function:initialize': async () => {
        ...
        this.rejectDurableFunctionDeploy();
      },
  ...
  rejectDurableFunctionDeploy() {
    const remoteDurableConfig =
      this.serverless.service.provider.remoteFunctionData.Configuration &&
      this.serverless.service.provider.remoteFunctionData.Configuration.DurableConfig;
    if (!this.options.functionObj.durableConfig && !remoteDurableConfig) return;
    throw new ServerlessError(
      `Function "${this.options.function}" is configured with durableConfig locally or in AWS. Use "serverless deploy" so osls can publish a new durable function version and update the generated alias.`,
      'DURABLE_DEPLOY_FUNCTION_UNSUPPORTED'
    );
  }
