...
class AwsDeployFunction {
  ...
  async checkIfFunctionExists() {
    ...
    const result = await (async () => {
      try {
        return await this.provider._awsRequest('Lambda', 'getFunction', params);
      } catch (error) {
  ...
  async normalizeArnRole(role) {
    ...
    const data = await this.provider._awsRequest('IAM', 'getRole', {
  ...
  async ensureFunctionState() {
    ...
    const callWithRetry = async () => {
      const result = await this.provider._awsRequest('Lambda', 'getFunction', params);
      if (
        result &&
  ...
  async callUpdateFunctionConfiguration(params) {
    ...
    const callWithRetry = async () => {
      try {
        await this.provider._awsRequest('Lambda', 'updateFunctionConfiguration', params);
      } catch (err) {
        const didOneMinutePass = Date.now() - startTime > 60 * 1000;
  ...
  async deployFunction() {
    ...
    await this.provider._awsRequest('Lambda', 'updateFunctionCode', params);
    this.shouldEnsureFunctionState = true;
    log.notice();
    log.notice.success(
