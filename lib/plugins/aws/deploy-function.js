...
class AwsDeployFunction {
  ...
  async checkIfFunctionExists() {
    ...
    const result = await (async () => {
      try {
        ...
        if (error.providerError && error.providerError.code === 'ResourceNotFoundException') {
          const errorMessage = [
  ...
  async updateFunctionConfiguration() {
    ...
    if (
      ...
      ((remoteFunctionConfiguration.DeadLetterConfig &&
        remoteFunctionConfiguration.DeadLetterConfig.TargetArn) ||
        null) !== functionObj.onError
    ) {
    ...
    if (
      params.Environment &&
      remoteFunctionConfiguration.Environment &&
      isDeepStrictEqual(params.Environment.Variables, remoteFunctionConfiguration.Environment.Variables)
    ) {
      delete params.Environment;
    }
    ...
    if (functionObj.image) {
      ...
      if (
        !isDeepStrictEqual(
          imageConfig,
          (remoteFunctionConfiguration.ImageConfigResponse &&
            remoteFunctionConfiguration.ImageConfigResponse.ImageConfig) ||
            {}
        )
      ) {
