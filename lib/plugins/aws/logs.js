...
class AwsLogs {
  ...
  /**
   * Helper method to route CloudWatchLogs requests through v2 or v3 based on feature flag
   * @private
   */
  async _cloudWatchLogsRequest(method, params) {
    // Use v3 if feature flag is enabled, otherwise fallback to v2
    if (this.provider._v3Enabled) {
      return this.provider.requestV3('CloudWatchLogs', method, params);
    }
  ...
  async getLogStreams() {
    ...
    const reply = await this._cloudWatchLogsRequest('describeLogStreams', params);
  ...
  async showLogs(logStreamNames) {
    ...
    const results = await this._cloudWatchLogsRequest('filterLogEvents', params);
    if (results.events) {
