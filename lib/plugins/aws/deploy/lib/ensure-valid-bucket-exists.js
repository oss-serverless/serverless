...
module.exports = {
  async ensureValidBucketExists() {
    ...
    if (this.serverless.service.provider.deploymentBucket) {
      let result;
      try {
        result = await this.provider._awsRequest('S3', 'getBucketLocation', {
    ...
    const getTemplateResult = await this.provider._awsRequest('CloudFormation', 'getTemplate', {
      StackName: stackName,
    ...
    if (this.serverless.service.provider.deploymentMethod === 'direct') {
      ...
      const changeSet = await this.provider._awsRequest(
        'CloudFormation',
        'createChangeSet',
