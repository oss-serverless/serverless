...
module.exports = {
  ...
  async ensureValidBucketExists() {
    ...
    if (this.serverless.service.provider.deploymentBucket) {
      let result;
      try {
        result = await this._awsRequest('S3', 'getBucketLocation', {
    ...
    const getTemplateResult = await this._awsRequest('CloudFormation', 'getTemplate', {
      StackName: stackName,
    ...
    if (this.serverless.service.provider.deploymentMethod === 'direct') {
      ...
      monitorCfData = await this._awsRequest('CloudFormation', 'updateStack', params);
    } else {
      ...
      await this._awsRequest('CloudFormation', 'deleteChangeSet', {
        StackName: stackName,
      ...
      const changeSet = await this._awsRequest(
        'CloudFormation',
        'createChangeSet',
      ...
      await this._awsRequest('CloudFormation', 'executeChangeSet', executeChangeSetParams);
