...
function getDeploymentDirectoryTimestamp(key) {
  const directory = key.split('/')[3];
...
module.exports = {
  ...
  async getMostRecentObjects() {
    ...
    const ordered = [...objects].sort((objectA, objectB) => {
      const timestampDiff =
        getDeploymentDirectoryTimestamp(objectB.Key) - getDeploymentDirectoryTimestamp(objectA.Key);
      if (timestampDiff) return timestampDiff;
      return objectB.Key.localeCompare(objectA.Key);
    });
