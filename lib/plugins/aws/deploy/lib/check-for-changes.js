...
module.exports = {
  ...
  async getFunctionsEarliestLastModifiedDate() {
    ...
    const getFunctionResults = this.serverless.service.getAllFunctions().map((funName) => {
      ...
      return this.provider
        ._awsRequest('Lambda', 'getFunction', {
  ...
  async getObjectMetadata(objects) {
    return Promise.all(
      objects.map(async (obj) => {
        try {
          const result = await this.provider._awsRequest('S3', 'headObject', {
            Bucket: this.bucketName,
            Key: obj.Key,
          });
          result.Key = obj.Key;
  ...
  async fixLogGroupSubscriptionFilters(params) {
    ...
    const response = await this.provider
      ._awsRequest('CloudWatchLogs', 'describeSubscriptionFilters', {
        logGroupName,
      })
    ...
    return Promise.all(
      notMatchedInternalOldSubscriptionFilters.map((oldSubscriptionFilter) =>
        this.provider._awsRequest('CloudWatchLogs', 'deleteSubscriptionFilter', {
          logGroupName,
  ...
  async isInternalSubscriptionFilter(stackName, logicalResourceId, physicalResourceId) {
    try {
      const { StackResourceDetail } = await this.provider._awsRequest(
        'CloudFormation',
