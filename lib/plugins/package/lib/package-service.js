...
module.exports = {
  ...
  getIncludes(include) {
    const packageIncludes = [...(this.serverless.service.package.patterns || [])];
    return Array.from(new Set([...packageIncludes, ...(include || [])]));
  },
    ...
    let packagePromises = allFunctions.map(async (functionName) => {
      ...
      if (functionObject.package.patterns && !shownFuncPackageWarning) {
        ...
        const warningMessage = [
          ...
          ' osls will ignore the patterns defined at the function level and apply only the service-wide ones.',
        ].join('');
  ...
  async resolveFilePathsLayer(layerName) {
    ...
    return this.resolveFilePathsFromPatterns(
      await this.excludeDevDependencies({
        exclude: this.getExcludes([], false),
  ...
  async resolveFilePathsFromPatterns(params, prefix) {
    ...
    return glob(['**'].concat(params.include), {
      ...
      const filePathStates = allFilePaths.reduce((state, filePath) => {
        state[filePath] = true;
        return state;
      }, createRegistry());
      patterns
