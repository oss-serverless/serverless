...
if (!fs.readFileAsync) fs.readFileAsync = (...args) => fsp.readFile(...args);
...
async function excludeNodeDevDependencies(serviceDir) {
  ...
  try {
    ...
    for (const env of ['dev', 'prod']) {
      const depFile = env === 'dev' ? nodeDevDepFile : nodeProdDepFile;
      try {
        const fileContent = await fs.readFileAsync(depFile);
    ...
    if (dependencies.length) {
      ...
      for (const dependency of dependencies) {
        ...
        try {
          const packageJson = JSON.parse(await fs.readFileAsync(packagePath, 'utf-8'));
