...
const { UNSAFE_PROPERTY_KEYS, hasOwn, isUnsafePropertyKey } = require('../../utils/safe-object');
  ...
  if (configuration == null || typeof configuration !== 'object') {
    throw new ServerlessError(
      `Resolved "${configurationFilename}" does not contain valid Compose configuration.\n` +
        'Read about Serverless Framework Compose in the documentation: https://github.com/oss-serverless/serverless/blob/main/docs/guides/compose.md',
  ...
  if (!hasOwn(configuration, 'services') || !isObject(configuration.services)) {
    throw new ServerlessError(
      `Invalid configuration: "${configurationFilename}" must contain "services" property.\n` +
        'Read about Serverless Framework Compose configuration in the documentation: https://github.com/oss-serverless/serverless/blob/main/docs/guides/compose.md',
    ...
    if (isUnsafePropertyKey(key)) {
      throw new ServerlessError(
        `Invalid configuration: definition of "${key}" service uses a reserved alias. ` +
          `Reserved aliases: ${Array.from(UNSAFE_PROPERTY_KEYS).join(', ')}.`,
    ...
    if (hasOwn(configuration, key)) {
      throw new ServerlessError(
        `Invalid property "${key}" in "${configurationFilename}".\n` +
          ...
          'You can search and/or open feature requests here: https://github.com/oss-serverless/serverless/blob/main/docs/guides/compose.md',
