...
const { createRegistry, hasOwn, isUnsafePropertyKey } = require('../../utils/safe-object');

const ensureValidComponentId = (componentId) => {
  if (!isUnsafePropertyKey(componentId)) return;
...
class BaseStateStorage {
  ...
  async readComponentState(componentId) {
    ...
    if (isUnsafePropertyKey(componentId)) {
      return {};
    }
  ...
  async readComponentsOutputs() {
    ...
    for (const [id, data] of Object.entries(this.state.components)) {
      if (isUnsafePropertyKey(id)) continue;
