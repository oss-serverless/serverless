...
const visitLeaves = require('./utils/visit-leaves');
...
const resolveObject = (object, context, method) => {
  ...
  const resolvedObject = visitLeaves(object, (value) => {
...
const getAllComponents = async (obj = {}, root = process.cwd()) => {
  ...
  for (const [key, val] of Object.entries(obj.services)) {
    ...
    if (val.component[0] === '.') {
      ...
      if (!(await fileExists(localComponentPath))) {
        throw new ServerlessError(
...
const setDependencies = (allComponents) => {
  ...
  for (const alias of Object.keys(allComponents)) {
    ...
    visitLeaves(allComponents[alias].inputs, (value) => {
      const matches = typeof value === 'string' ? value.match(regex) : null;
      if (!matches) return undefined;
      for (const match of matches) {
        ...
        dependencies.add(referencedComponent);
      }
      return undefined;
    });
...
class ComponentsService {
  ...
  async invokeComponentCommand(componentName, command, options) {
    if (isUnsafePropertyKey(componentName)) {
      throw new ServerlessError(
