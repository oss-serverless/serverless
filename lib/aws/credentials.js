...
const configProfileSectionRegex = /^profile\s(["'])?([\w-@+.%:/]+)\1$/;
...
function getConfigProfileName(sectionName) {
  if (sectionName === 'default') return 'default';

  const match = configProfileSectionRegex.exec(sectionName);
  if (!match) return null;

  return match[2];
}

function getConfigProfileNames(filePath) {
  const sectionNames = getIniSectionNames(filePath);
  const profileNames = new Set();

  for (const sectionName of sectionNames) {
    const profileName = getConfigProfileName(sectionName);
...
function doesProfileExist(profile) {
  ...
  const configProfiles = getConfigProfileNames(getSharedConfigFilepath());

  return credentialsProfiles.has(profile) || configProfiles.has(profile);
}
