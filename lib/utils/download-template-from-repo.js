...
async function downloadTemplateFromRepo(inputUrl, requestedServiceName, downloadPath) {
  return parseRepoURL(inputUrl).then((repoInformation) => {
    ...
    const targetDirectoryName = downloadPath || requestedServiceName || sourceName;
    const targetDirectoryPath = path.resolve(process.cwd(), untildify(targetDirectoryName));
    const effectiveServiceName =
      requestedServiceName || (downloadPath ? path.basename(targetDirectoryPath) : sourceName);
    const shouldRenameService = Boolean(requestedServiceName || downloadPath);
    ...
    if (dirExistsSync(targetDirectoryPath)) {
      const errorMessage = `A folder named "${targetDirectoryPath}" already exists.`;
    ...
    return download(repoInformation.downloadUrl, downloadServicePath, downloadOptions)
      ...
      .then(() => {
        ...
        return BbPromise.resolve(sourceName);
      });
  });
}
