  ...
  constructor(config = {}) {
    ...
    if (isStackFromOldLocalFallback(new Error().stack)) {
      ...
      throw new ServerlessError(
        [
          'Cannot run local installation of osls with the outdated global version.',
          'Please upgrade via:',
          '',
          colors.bold(
            isStandaloneExecutable ? 'serverless upgrade --major' : 'npm install -g osls'
          ),
          colors.gray('Note: Latest release can run any version of the locally installed osls.'),
          '',
