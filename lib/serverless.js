...
// Fix issues with node 22+ https://github.com/mathiasbynens/punycode.js/issues/137
const moduleAlias = require('module-alias');
const path = require('path');

moduleAlias.addAlias('punycode', 'punycode/');

const vendoredServerlessUtilsPath = path.resolve(__dirname, 'utils/serverless-utils');
for (const [request, target] of Object.entries({
  '@serverless/utils/config': 'config.js',
  '@serverless/utils/config.js': 'config.js',
  '@serverless/utils/log': 'log.js',
  '@serverless/utils/log.js': 'log.js',
})) {
  moduleAlias.addAlias(request, path.join(vendoredServerlessUtilsPath, target));
}
...
class Serverless {
  constructor(config = {}) {
    ...
    if (isStackFromOldLocalFallback(new Error().stack)) {
      // Prevent old local fallback, as it's not compatible with this version of the CLI
      const colors = require('./utils/colors');
      throw new ServerlessError(
        [
          'Cannot run local installation of OSLS Framework with the outdated global version.',
          'Please upgrade via:',
          '',
          colors.bold(
          ...
          colors.gray(
            'Note: Latest release can run any version of the locally installed OSLS Framework.'
          ),
          '',
          'Alternatively run locally installed version directly via:',
          '',
          colors.bold('npx serverless <command> <options>'),
        ].join('\n'),
    ...
    if (hasOwn(cliOptions, 'stage')) validateStage(cliOptions.stage);
    this.processedInput = {
  ...
  extendConfiguration(configurationPathKeys, value) {
    ...
    if (!setByPath(this.configurationInput, configurationPathKeys, value, { arrays: true })) {
      return;
    }
