...
const groupBy = (items, iteratee) => {
  // Use a null-prototype accumulator so that unusual key values cannot collide
  // with inherited Object.prototype members (e.g., result['__proto__'] would
  // otherwise return Object.prototype and break `||= []` + push).
...
const filterIrreleventEventConfigurationErrors = (resultErrorsSet) => {
  ...
  const eventTypeErrorsByEvent = groupBy(eventTypeErrors, ({ instancePath }) => instancePath);
  ...
  for (const [instancePath, eventEventTypeErrors] of Object.entries(eventTypeErrorsByEvent)) {
    ...
    const eventEventTypeErrorsByTypeIndex = groupBy(eventEventTypeErrors, ({ schemaPath }) => {
...
const filterIrrelevantAnyOfErrors = (resultErrorsSet) => {
  ...
  for (const [oneOfPath, oneOfPathErrors] of Object.entries(oneOfErrorsByPath)) {
    ...
    Object.values(
      groupBy(oneOfPathErrors, ({ instancePath }) => {
        if (
      ...
      const instancePathOneOfPathErrorsByVariant = groupBy(
