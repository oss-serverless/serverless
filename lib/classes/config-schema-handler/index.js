...
class ConfigSchemaHandler {
  ...
  defineTopLevelProperty(name, subSchema) {
    if (this.schema.properties[name]) {
      throw new ServerlessError(
        `Top-level property '${name}' already have a definition - this property might have already been defined by osls or one other plugin`,
        'SCHEMA_COLLISION'
      );
    }
  ...
  defineProvider(name, options = {}) {
    ...
    if (options.provider) {
      try {
        ...
        if (error instanceof PropertyCollisionError) {
          throw new ServerlessError(
            `Property 'provider.${error.property}' already have a definition - this property might have already been defined by osls or one other plugin`,
            'SCHEMA_COLLISION'
          );
        }
        throw error;
    ...
    if (options.function) {
      try {
        ...
        if (error instanceof PropertyCollisionError) {
          throw new ServerlessError(
            `Property 'functions[].${error.property}' already have a definition - this property might have already been defined by osls or one other plugin`,
            'SCHEMA_COLLISION'
          );
        }
        throw error;
    ...
    // osls internally sets these options in Service class.
  ...
  defineCustomProperties(configSchemaParts) {
    try {
      ...
      if (error instanceof PropertyCollisionError) {
        throw new ServerlessError(
          `Property 'custom.${error.property}' already have a definition - this property might have already been defined by osls or one other plugin`,
          'SCHEMA_COLLISION'
        );
      }
      throw error;
  ...
  defineFunctionEvent(providerName, name, configSchema) {
    ...
    if (existingFunctionEvents.has(name)) {
      throw new ServerlessError(
        `Function event '${name}' already have a definition - this event might have already been defined by osls or one other plugin`,
        'SCHEMA_COLLISION'
      );
    }
  ...
  defineFunctionEventProperties(providerName, name, configSchema) {
    ...
    try {
      ...
      if (error instanceof PropertyCollisionError) {
        throw new ServerlessError(
          `Property 'functions[].events[].${name}.${error.property}' already have a definition - this property might have already been defined by osls or one other plugin`,
          'SCHEMA_COLLISION'
        );
      }
      throw error;
