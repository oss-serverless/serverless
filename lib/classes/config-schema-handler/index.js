...
const { log, style } = require('../../utils/serverless-utils/log');
...
const normalizeSchemaObject = (object, instanceSchema) => {
  for (const [key, value] of Object.entries(object)) {
    ...
    object[key] = getOwnByPath(instanceSchema, value.$ref.slice(2).split('/'));
  }
};
  ...
  const normalizeObject = (object, path) => {
    ...
    if (Array.isArray(object)) {
      for (const [index, value] of object.entries()) {
        if (isObject(value)) normalizeObject(value, path.concat(index));
      }
    } else {
      for (const [key, value] of Object.entries(object)) {
        if (value == null) {
...
const denormalizeUserConfig = (userConfig, { removedValuesMap }) => {
  for (const removedValueData of removedValuesMap) {
    setByPath(userConfig, removedValueData.path, removedValueData.value, { arrays: true });
  }
};
...
class ConfigSchemaHandler {
    ...
    if (messages.length) {
      if (this.serverless.service.configValidationMode === 'error') {
        throw new ServerlessError(
          ...
          }\n\nLearn more about configuration validation here: https://github.com/oss-serverless/framework/blob/main/docs/guides/configuration-validation.md`,
        ...
        log.warning(
          [
            ...
            `Learn more about configuration validation here: ${style.link(
              'https://github.com/oss-serverless/framework/blob/main/docs/guides/configuration-validation.md'
            )}`,
          ].join('\n')
        );
        if (!this.serverless.configurationInput.configValidationMode) {
          this.serverless._logDeprecation(
            ...
            'Starting with the next major, OSLS Framework will throw on configuration errors by' +
  ...
  defineTopLevelProperty(name, subSchema) {
    if (this.schema.properties[name]) {
      throw new ServerlessError(
        `Top-level property '${name}' already have a definition - this property might have already been defined by the OSLS Framework or one other plugin`,
        'SCHEMA_COLLISION'
      );
    }
  ...
  defineProvider(name, options = {}) {
    ...
    if (options.provider) {
      try {
        ...
        if (error instanceof PropertyCollisionError) {
          throw new ServerlessError(
            `Property 'provider.${error.property}' already have a definition - this property might have already been defined by the OSLS Framework or one other plugin`,
            'SCHEMA_COLLISION'
          );
        }
        throw error;
    ...
    if (options.function) {
      try {
        ...
        if (error instanceof PropertyCollisionError) {
          throw new ServerlessError(
            `Property 'functions[].${error.property}' already have a definition - this property might have already been defined by the OSLS Framework or one other plugin`,
            'SCHEMA_COLLISION'
          );
        }
        throw error;
  ...
  defineCustomProperties(configSchemaParts) {
    try {
      ...
      if (error instanceof PropertyCollisionError) {
        throw new ServerlessError(
          `Property 'custom.${error.property}' already have a definition - this property might have already been defined by the OSLS Framework or one other plugin`,
          'SCHEMA_COLLISION'
        );
      }
      throw error;
  ...
  defineFunctionEvent(providerName, name, configSchema) {
    ...
    if (existingFunctionEvents.has(name)) {
      throw new ServerlessError(
        `Function event '${name}' already have a definition - this event might have already been defined by the OSLS Framework or one other plugin`,
        'SCHEMA_COLLISION'
      );
    }
  ...
  defineFunctionEventProperties(providerName, name, configSchema) {
    ...
    try {
      ...
      if (error instanceof PropertyCollisionError) {
        throw new ServerlessError(
          `Property 'functions[].events[].${name}.${error.property}' already have a definition - this property might have already been defined by the OSLS Framework or one other plugin`,
          'SCHEMA_COLLISION'
        );
      }
      throw error;
  ...
  relaxProviderSchema() {
    ...
    // functions[]
    this.schema.properties.functions.patternProperties[FUNCTION_NAME_PATTERN].additionalProperties =
      true;
