...
class PluginManager {
  ...
  async requireServicePlugin(serviceDir, pluginPath, legacyLocalPluginsPath) {
    ...
    // Search in the "node_modules" where OSLS Framework is placed
  ...
  async resolveServicePlugins(servicePlugs) {
    ...
    for (const name of pluginNames) {
      ...
      try {
        ...
        throw new ServerlessError(
          [
            `OSLS Framework plugin "${name}" not found.`,
      ...
      if (!Plugin) {
        throw new ServerlessError(
          `OSLS Framework plugin "${name}", didn't export Plugin constructor.`,
          'MISSING_PLUGIN_NAME'
        );
      }
  ...
  getCommand(commandsArray, allowEntryPoints) {
    ...
    return commandOrAlias.reduce(
      (current, name, index) => {
        ...
        // Invalid command, can happen only when OSLS Framework is used programmatically,
