...
const getCommandByPath = (commands, commandPath) => {
  const [head, ...rest] = commandPath.split(':');
  let current = commands[head];

  for (const name of rest) {
    if (!current || !current.commands) return undefined;
...
class PluginManager {
  ...
  addPlugin(Plugin) {
    ...
    if (pluginInstance.provider) {
      ...
      } else if (isObject(pluginInstance.provider)) {
  ...
  async loadAllPlugins(servicePlugins) {
    ...
    const configuredPluginNames = new Set(
      Array.isArray(servicePlugins)
        ? servicePlugins
        : Array.isArray(servicePlugins?.modules)
          ? servicePlugins.modules
          : []
    );
    require('../plugins')
      .filter(Boolean)
      .filter(
        (Plugin) =>
          !Plugin.bundledPluginName || !configuredPluginNames.has(Plugin.bundledPluginName)
      )
      .filter(
        (Plugin) =>
          !Plugin.shouldLoad ||
  ...
  async requireServicePlugin(serviceDir, pluginPath, legacyLocalPluginsPath) {
    ...
    // Search in the "node_modules" where osls is placed
  ...
  async resolveServicePlugins(servicePlugs) {
    ...
    for (const name of pluginNames) {
      ...
      if (!Plugin) {
        throw new ServerlessError(
          `osls plugin "${name}", didn't export Plugin constructor.`,
          'MISSING_PLUGIN_NAME'
        );
      }
  ...
  loadCommand(pluginName, details, key, isEntryPoint) {
    ...
    const commands = Object.fromEntries(
      Object.entries(details.commands || {}).map(([subKey, subDetails]) => [
        subKey,
      ...
      if (commands) {
        Object.entries(commands).forEach(([name, command]) => {
          if (command.type !== 'entrypoint') {
            const { commands: childCommands, ...publicCommand } = command;
            target[name] = publicCommand;
            if (
              Object.values(childCommands || {}).some(
                (childCommand) => childCommand.type !== 'entrypoint'
              )
            ) {
              target[name].commands = {};
              stack.push({ commands: childCommands, target: target[name].commands });
            }
          }
        });
      }
    ...
    while (stack.length) {
      ...
      if (aliases) {
        Object.entries(aliases).forEach(([name, alias]) => {
          ...
          if (alias.command) {
            const command = getCommandByPath(this.commands, alias.command);
            if (!command) return;
            target[name] = command;
          } else {
  ...
  getCommand(commandsArray, allowEntryPoints) {
    ...
    return commandOrAlias.reduce(
      (current, name, index) => {
        ...
        // Invalid command, can happen only when osls is used programmatically,
  ...
  async spawn(commandsArray, options) {
    ...
    if (options && options.terminateLifecycleAfterExecution) {
  ...
  validateServerlessConfigDependency(command) {
    if (command.configDependent || command.serviceDependencyMode === 'required') {
      if (!this.serverless.configurationInput) {
        const msg = [
          'This command can only be run in a service directory. ',
